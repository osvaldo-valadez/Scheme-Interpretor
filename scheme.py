...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        check_procedure(op)
        oper=rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(op,oper,env)
        # END PROBLEM 5
...
class Frame(object):
    ...
    def define(self, symbol, value):
        ...
        self.symbol=symbol
        self.value=value
        self.bindings[self.symbol]=self.value
        # END PROBLEM 3
    ...
    def lookup(self, symbol):
        ...
        # BEGIN PROBLEM 3
        self.symbol = symbol
        if self.symbol in self.bindings:
            return self.bindings[self.symbol]
        else:
            if self.parent != None:
                return self.parent.lookup(self.symbol)
            else:
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 12
        self.args = args
        return self.env.make_child_frame(self.formals, self.args)
        # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    if expressions.second == nil:
        return scheme_eval(expressions.first,env)
    EV = scheme_eval(expressions.first, env)
    if scheme_truep(EV):
        return EV
    else:
